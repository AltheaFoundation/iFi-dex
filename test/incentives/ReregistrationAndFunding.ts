...
describe('ReregistrationAndFunding - Per-Block Incentives', () => {
    ...
    describe("Re-registration", () => {
        ...
        it("can register without prior registration (initial registration)", async () => {
            ...
            const userInfo = await incentives.getConcentratedUserInfo(traderAddress, baseQuotePoolId, rewardToken.address);
    ...
    describe("Funding and Program End", () => {
        ...
        it("cannot register for exhausted program", async () => {
            ...
            // Now try to re-register - should fail
            await expect(
                incentives.register(baseQuotePoolId, rewardToken.address, true)
        ...
        it("can modify and refund exhausted program", async () => {
            ...
            const programBefore = await incentives.getConcentratedProgramInfo(baseQuotePoolId, rewardToken.address);
            ...
            const programAfter = await incentives.getConcentratedProgramInfo(baseQuotePoolId, rewardToken.address);
        ...
        it("modifying program extends endBlock with new funding", async () => {
            ...
            const program1 = await incentives.getConcentratedProgramInfo(baseQuotePoolId, rewardToken.address);
            ...
            const program2 = await incentives.getConcentratedProgramInfo(baseQuotePoolId, rewardToken.address);
        ...
        it("accounts for pending rewards when calculating available balance", async () => {
            ...
            const program = await incentives.getConcentratedProgramInfo(base2QuotePoolId, rewardToken.address);
            const currentBlock = await ethers.provider.getBlockNumber();
    ...
    describe("View Functions", () => {
        ...
        it("isConcentratedProgramActive returns correct status", async () => {
            const rewardPerBlock = ethers.utils.parseUnits("10", 18);
            ...
            let isActive = await incentives.isConcentratedProgramActive(baseQuotePoolId, rewardToken.address);
            expect(isActive).to.be.true;

            // Wait past end block
            await hardhat.network.provider.send("hardhat_mine", [`0x${(20).toString(16)}`]);

            isActive = await incentives.isConcentratedProgramActive(baseQuotePoolId, rewardToken.address);
            expect(isActive).to.be.false;
        });

        it("getConcentratedProgramInfo returns all program details", async () => {
            const rewardPerBlock = ethers.utils.parseUnits("10", 18);
