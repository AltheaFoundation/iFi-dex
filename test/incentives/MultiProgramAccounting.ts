...
describe('MultiProgramAccounting - Per-Block Incentives', () => {
    ...
    describe("Multiple Concentrated Programs Same Token", () => {
        it("two concentrated programs with same reward token track commitments correctly", async () => {
            ...
            const program1 = await incentives.getProgramInfo(pool1Id, rewardToken.address, true);
            ...
            const program2 = await incentives.getProgramInfo(pool2Id, rewardToken.address, true);
            ...
            // Verify both programs are active (not exhausted)
            const isActive1 = await incentives.isProgramActive(pool1Id, rewardToken.address, true);
            const isActive2 = await incentives.isProgramActive(pool2Id, rewardToken.address, true);
        ...
        it("programs operate independently with same reward token", async () => {
            ...
            // Register for both
            await incentives.register(traderAddress, pool1Id, rewardToken.address, true);
            await incentives.register(traderAddress, pool2Id, rewardToken.address, true);
    ...
    describe("Multiple Ambient Programs Same Token", () => {
        it("two ambient programs with same reward token work independently", async () => {
            ...
            await incentives.register(traderAddress, pool1Id, rewardToken.address, false);
            await incentives.connect(userB).register(await userB.getAddress(), pool1Id, rewardToken.address, false);
    ...
    describe("Mixed Concentrated and Ambient Programs", () => {
        it("concentrated and ambient programs with same reward token work independently", async () => {
            ...
            await incentives.register(traderAddress, pool1Id, rewardToken.address, true);
            await incentives.register(traderAddress, pool2Id, rewardToken.address, false);
    ...
    describe("Same Pool Multiple Reward Tokens", () => {
        it("concentrated: same pool can have multiple different reward tokens", async () => {
            ...
            // Register for both
            await incentives.register(traderAddress, pool1Id, rewardToken.address, true);
            await incentives.register(traderAddress, pool1Id, rewardToken2.address, true);
        ...
        it("ambient: same pool can have multiple different reward tokens", async () => {
            ...
            await incentives.register(traderAddress, pool1Id, rewardToken.address, false);
            await incentives.register(traderAddress, pool1Id, rewardToken2.address, false);
    ...
    describe("Funding Exhaustion with Multiple Programs", () => {
        it("one program exhausting doesn't affect other programs", async () => {
            ...
            const isPool2Active = await incentives.isProgramActive(pool2Id, rewardToken.address, true);
