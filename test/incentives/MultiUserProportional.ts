...
describe('MultiUserProportional - Per-Block Incentives', () => {
    ...
    it("three users across multiple block periods with staggered claims", async () => {
        ...
        const pendingA = await incentives.getPendingConcentratedRewards(baseQuotePoolId, userAAddress, rewardToken.address);
    ...
    it("mid-block liquidity changes affect reward distribution continuously", async () => {
        ...
        const pendingA = await incentives.getPendingConcentratedRewards(baseQuotePoolId, userAAddress, rewardToken.address);
        const pendingB = await incentives.getPendingConcentratedRewards(baseQuotePoolId, userBAddress, rewardToken.address);
        ...
        const pendingA_period2 = await incentives.getPendingConcentratedRewards(baseQuotePoolId, userAAddress, rewardToken.address);
        const pendingB_period2 = await incentives.getPendingConcentratedRewards(baseQuotePoolId, userBAddress, rewardToken.address);
    ...
    it("users with different liquidity amounts over multiple block periods", async () => {
        ...
        const pendingA_after3 = await incentives.getPendingConcentratedRewards(baseQuotePoolId, userAAddress, rewardToken.address);
