...
describe('ValidationTests - Per-Block Incentives', () => {
    ...
    describe("View Function Edge Cases", () => {
        it("getPendingConcentratedRewards returns 0 for non-existent program", async () => {
            const traderAddress = await (await test1.trader).getAddress();
            
            const pending = await incentives.getPendingConcentratedRewards(
                baseQuotePoolId,
                traderAddress,
                rewardToken.address
            );
        ...
        it("getPendingAmbientRewards returns 0 for non-existent program", async () => {
            const traderAddress = await (await test1.trader).getAddress();
            
            const pending = await incentives.getPendingAmbientRewards(
                baseQuotePoolId,
                traderAddress,
                rewardToken.address
            );
        ...
        it("getPendingConcentratedRewards returns 0 when user not registered", async () => {
            const fundingAmount = ethers.utils.parseUnits("1000", 18);
            const traderAddress = await (await test1.trader).getAddress();
            await rewardToken.contract.deposit(traderAddress, fundingAmount);
            await rewardToken.approve(await test1.trader, await incentives.address, fundingAmount);
        ...
        it("getPendingAmbientRewards returns 0 when user not registered", async () => {
            const fundingAmount = ethers.utils.parseUnits("1000", 18);
            const traderAddress = await (await test1.trader).getAddress();
            await rewardToken.contract.deposit(traderAddress, fundingAmount);
            await rewardToken.approve(await test1.trader, await incentives.address, fundingAmount);
        ...
        it("getConcentratedProgramInfo works for non-existent program", async () => {
            const program = await incentives.getConcentratedProgramInfo(
                baseQuotePoolId,
                rewardToken.address
            );
        ...
        it("getAmbientProgramInfo works for non-existent program", async () => {
            const program = await incentives.getAmbientProgramInfo(
                baseQuotePoolId,
                rewardToken.address
            );
        ...
        it("isConcentratedProgramActive returns false for non-existent program", async () => {
            const isActive = await incentives.isConcentratedProgramActive(
                baseQuotePoolId,
                rewardToken.address
            );
        ...
        it("isAmbientProgramActive returns false for non-existent program", async () => {
            const isActive = await incentives.isAmbientProgramActive(
                baseQuotePoolId,
                rewardToken.address
            );
        ...
        it("getConcentratedRemainingBlocks returns 0 for non-existent program", async () => {
            const remaining = await incentives.getConcentratedRemainingBlocks(
                baseQuotePoolId,
                rewardToken.address
            );
        ...
        it("getConcentratedRemainingRewards returns 0 for non-existent program", async () => {
            const remaining = await incentives.getConcentratedRemainingRewards(
                baseQuotePoolId,
                rewardToken.address
            );
