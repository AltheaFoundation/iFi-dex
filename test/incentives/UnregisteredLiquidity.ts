...
describe('Unregistered Liquidity Impact', () => {
    ...
    it("demonstrates reward dilution from unregistered liquidity", async function() {
        ...
        await incentives.connect(userRegistered).register(poolId, rewardToken.address, false);
    ...
    it("demonstrates accumulator behavior with no registered users", async function() {
        ...
        // Check program state
        const programInfo = await incentives.getAmbientProgramInfo(pool2Id, rewardToken.address);
    ...
    it("demonstrates late registration disadvantage", async function() {
        ...
        await incentives.connect(userEarly).register(pool3Id, rewardToken.address, false);
        ...
        await incentives.connect(userLate).register(pool3Id, rewardToken.address, false);
