    ...
    switch (action.actionType) {
        case ActionType.ADD_LIQUIDITY:
            ...
            if (!userState.registered) {
                ...
                if (isConcentrated) {
                    tx = await incentives.connect(wallet).register(poolId, rewardToken.address, true);
                    lastReceipt = await tx.wait();
                } else {
                    tx = await incentives.connect(wallet).register(poolId, rewardToken.address, false);
                    lastReceipt = await tx.wait();
                }
