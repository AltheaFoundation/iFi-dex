...
describe('ViewFunctions - Per-Block Incentives', () => {
    ...
    describe("Program Status Functions", () => {
        it("getProgramStatus returns accurate data (concentrated)", async () => {
            const rewardPerBlock = ethers.utils.parseUnits("10", 18);
            ...
            const status = await incentives.getProgramStatus(baseQuotePoolId, rewardToken.address, true);
            
            expect(status.active).to.be.true; // Active because not yet exhausted
        ...
        it("getProgramStatus returns accurate data (ambient)", async () => {
            const rewardPerBlock = ethers.utils.parseUnits("10", 18);
            ...
            const status = await incentives.getProgramStatus(baseQuotePoolId, rewardToken.address, false);
        ...
        it("status reflects exhaustion correctly", async () => {
            ...
            expect(status.active).to.be.false; // No longer active (exhausted)
    ...
    describe("Remaining Blocks and Rewards", () => {
        it("concentrated: getRemainingBlocks decreases over time", async () => {
            const rewardPerBlock = ethers.utils.parseUnits("10", 18);
            const fundingAmount = ethers.utils.parseUnits("500", 18); // 50 blocks funded
            ...
            const remaining1 = await incentives.getRemainingBlocks(baseQuotePoolId, rewardToken.address, true);
            ...
            const remaining2 = await incentives.getRemainingBlocks(baseQuotePoolId, rewardToken.address, true);
            ...
            const remaining3 = await incentives.getRemainingBlocks(baseQuotePoolId, rewardToken.address, true);
            // Should have decreased further
            expect(remaining3.toNumber()).to.be.lessThan(remaining2.toNumber());
        });

        it("concentrated: getRemainingRewards matches remaining blocks", async () => {
            const rewardPerBlock = ethers.utils.parseUnits("10", 18);
            const fundingAmount = ethers.utils.parseUnits("500", 18); // 50 blocks funded
            ...
            const remainingBlocks = await incentives.getRemainingBlocks(baseQuotePoolId, rewardToken.address, true);
            const remainingRewards = await incentives.getRemainingRewards(baseQuotePoolId, rewardToken.address, true);
    ...
    describe("User Info Functions", () => {
        it("concentrated: getUserInfo returns correct data", async () => {
            const liq = 1000000;
            await test1.testMint(-5000, 8000, liq);
            ...
            // Before registration
            let userInfo = await incentives.getUserInfo(traderAddress, baseQuotePoolId, rewardToken.address, true);
            ...
            userInfo = await incentives.getUserInfo(traderAddress, baseQuotePoolId, rewardToken.address, true);
        ...
        it("ambient: getUserInfo returns correct data", async () => {
            const liq = 1000000;
            await test1.testMintAmbient(liq);
            ...
            let userInfo = await incentives.getUserInfo(traderAddress, baseQuotePoolId, rewardToken.address, false);
            expect(userInfo.registered).to.be.false;
            
            await incentives.register(traderAddress, baseQuotePoolId, rewardToken.address, false);
            
            userInfo = await incentives.getUserInfo(traderAddress, baseQuotePoolId, rewardToken.address, false);
            expect(userInfo.registered).to.be.true;
            // Liquidity is stored as liq * 1024 in the pool
            expect(userInfo.userLiquidity).to.equal(liq * 1024);
        });
