...
describe('TokenDecimalVariations - Per-Block Incentives', () => {
    ...
    it("concentrated: 6-decimal token (USDC) rewards work correctly", async () => {
        ...
        await incentives.register(baseQuotePoolId, rewardToken6.address, true);
        ...
        const pendingRewards = await incentives.getPendingConcentratedRewards(baseQuotePoolId, traderAddress, rewardToken6.address);
    ...
    it("concentrated: 8-decimal token (WBTC) rewards work correctly", async () => {
        ...
        // Register for rewards
        await incentives.register(baseQuotePoolId, rewardToken8.address, true);

        // Wait for 10 blocks
        await hardhat.network.provider.send("hardhat_mine", [`0x${(10).toString(16)}`]);

        // Check pending rewards - allow tolerance for transaction timing
        const pendingRewards = await incentives.getPendingConcentratedRewards(baseQuotePoolId, traderAddress, rewardToken8.address);
    ...
    it("ambient: 6-decimal token (USDC) rewards work correctly", async () => {
        ...
        await incentives.register(baseQuotePoolId, rewardToken6.address, false);

        // Wait for 10 blocks to accumulate rewards
        await hardhat.network.provider.send("hardhat_mine", [`0x${(10).toString(16)}`]);

        const pendingRewards = await incentives.getPendingAmbientRewards(baseQuotePoolId, traderAddress, rewardToken6.address);
    ...
    it("ambient: 8-decimal token (WBTC) rewards work correctly", async () => {
        ...
        await incentives.register(baseQuotePoolId, rewardToken8.address, false);

        // Wait for 10 blocks to accumulate rewards
        await hardhat.network.provider.send("hardhat_mine", [`0x${(10).toString(16)}`]);

        const pendingRewards = await incentives.getPendingAmbientRewards(baseQuotePoolId, traderAddress, rewardToken8.address);
    ...
    it("concentrated: mixed decimal tokens can run simultaneously", async () => {
        ...
        await incentives.register(baseQuotePoolId, rewardToken6.address, true);
        await incentives.register(baseQuotePoolId, rewardToken8.address, true);
        await incentives.register(baseQuotePoolId, rewardToken18.address, true);
